...
constexpr uint8_t BLS = 0x23;  // BLS rel      - Branch if Lower or Same (unsigned)
...
constexpr uint8_t BGE = 0x2C;  // BGE rel      - Branch if Greater or Equal (signed)
...
constexpr uint8_t U_OR_S = 0x40; // Bit 6: U (for PSHS/PULS) or S (for PSHU/PULU)
