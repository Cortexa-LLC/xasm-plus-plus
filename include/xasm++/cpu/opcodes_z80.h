...
constexpr uint8_t LD_HL_n =
    0x36; // LD (HL), n   - Store immediate to address in HL

// ============================================================================
// Load/Store Instructions - 16-bit
// ============================================================================

constexpr uint8_t LD_BC_nn =
    0x01; // LD BC, nn    - Load BC with immediate 16-bit
constexpr uint8_t LD_DE_nn =
    0x11; // LD DE, nn    - Load DE with immediate 16-bit
constexpr uint8_t LD_HL_nn =
    0x21; // LD HL, nn    - Load HL with immediate 16-bit
constexpr uint8_t LD_SP_nn =
    0x31; // LD SP, nn    - Load SP with immediate 16-bit

// LD HL, (nn) / LD (nn), HL - 16-bit memory operations
constexpr uint8_t LD_HL_addr =
    0x2A; // LD HL, (nn)  - Load HL from memory address
constexpr uint8_t LD_addr_HL =
    0x22; // LD (nn), HL  - Store HL to memory address
...
constexpr uint8_t ADC_A_n =
    0xCE; // ADC A, n     - Add immediate with carry to A
...
constexpr uint8_t SBC_A_n =
    0xDE; // SBC A, n     - Subtract immediate with carry from A
...
constexpr uint8_t RLCA =
    0x07; // RLCA         - Rotate Left Circular Accumulator
constexpr uint8_t RRCA =
...
constexpr uint8_t NEG =
    0x44; // NEG          - Negate accumulator (2's complement)
...
constexpr uint8_t SBC_HL_BC =
    0x42; // SBC HL, BC   - Subtract with carry BC from HL
constexpr uint8_t SBC_HL_DE =
    0x52; // SBC HL, DE   - Subtract with carry DE from HL
constexpr uint8_t SBC_HL_HL =
    0x62; // SBC HL, HL   - Subtract with carry HL from HL
constexpr uint8_t SBC_HL_SP =
    0x72; // SBC HL, SP   - Subtract with carry SP from HL
...
constexpr uint8_t LD_A_IX_d =
    0x7E; // LD A, (IX+d) - Load A from IX+displacement
...
constexpr uint8_t LD_A_IY_d =
    0x7E; // LD A, (IY+d) - Load A from IY+displacement
