...
**Beads Task:** xasm++-y7q
**Priority:** P2 (High - blocks disk imaging)
...
Fix xasm++ to produce byte-for-byte identical binaries to vasm for 100% binary compatibility.

### Background

POP disk imaging requires exact binary match. Current: 2/10 files match. Need: 100% match.

### Current State

Binary comparison vs vasm:
- ✓ BGDATA.bin - IDENTICAL (491 bytes)
- ✓ HRTABLES.bin - IDENTICAL (2480 bytes)
- ✗ EQ.bin - SIZE (xasm++ 7592 vs vasm 0 bytes)
- ✗ GAMEEQ.bin - SIZE (xasm++ 3401 vs vasm 0 bytes)
- ✗ HRPARAMS.bin - SIZE (xasm++ 2500 vs vasm 0 bytes)
- ✗ SEQTABLE.bin - CONTENT (2546 bytes, leading zeros)
- ✗ VERSION.bin - CONTENT (29 bytes, ASCII vs high-bit)

### Three Root Causes

1. **Equate-only files**: xasm++ outputs ORG range, vasm outputs 0 bytes
2. **ASC directive**: xasm++ plain ASCII (0x50='P'), vasm high-bit (0xD0='P'+0x80)
3. **Padding**: xasm++ pads from $0000 to ORG, vasm starts at ORG
...
```
✓ All 10 POP binaries match vasm byte-for-byte
✓ Equate-only files produce 0-byte output
✓ ASC directive sets high bit (Apple II format)
✓ No leading zero padding before ORG
✓ All existing tests still pass (729/729)
```
