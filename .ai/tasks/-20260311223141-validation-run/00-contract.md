# Contract: Validation Run — ssc.drv, ssc.i.drv, libtcpip, libetalk
...
Rebuild 4 A2osX files with the current xasm++ binary (post xasm-zp1 fixes) and compare against the stable reference to see if diff counts changed.

### Background

xasm-zp1 agent hit context limit at turn 239 after making two fixes:
1. **Double-dot label mis-parse** (`scmasm_syntax.cpp`): `X.BasePath..1` was mis-parsed as `X.BasePath.` + local `.1`. Fix: `at_word_start` is false when preceding char is `.`.
2. **Infix `*` multiplication** (`assembler.cpp`): `VALUE*SIZE` expressions now parsed as binary multiply.

These fixes may have resolved some or all of the 39/39/65/11 diffs in the 4 target files.

### Files

| File | Source | Stable | Prior diffs |
|------|--------|--------|-------------|
| drv/ssc.drv | /tmp/A2osX-335cd122/DRV/SSC.DRV.S.txt | /tmp/stable_60b98d24/ssc.drv#062000 | 39 |
| drv/ssc.i.drv | /tmp/A2osX-335cd122/DRV/SSC.I.DRV.S.txt | /tmp/stable_60b98d24/ssc.i.drv#062000 | 39 |
| lib/libtcpip | /tmp/A2osX-335cd122/LIB/LIBTCPIP.S.txt | /tmp/stable_60b98d24/libtcpip#062000 | 65 |
| lib/libetalk | /tmp/A2osX-335cd122/LIB/LIBETALK.S.txt | /tmp/stable_60b98d24/libetalk#062000 | 11 |

Build flags: `--cpu 65c02 --syntax scmasm -I /tmp/A2osX-335cd122/INC`
Binary: `cp build/bin/xasm++ /tmp/xasm_new`
Stage output: `/tmp/A2osX-335cd122-stage/drv/` and `/tmp/A2osX-335cd122-stage/lib/`

### Desired State

Diff report for all 4 files. Each classified as SYSCALL-only (source change) or POSSIBLE_BUG.
...
```
✓ All 4 files assembled without errors
✓ Diff report produced for each file (count, size delta, all positions, SYSCALL vs unexplained)
✓ Each file classified: SYSCALL-only OR POSSIBLE_BUG
✓ cmake --build build exits 0, ctest exits 0 (1835 tests)
```
