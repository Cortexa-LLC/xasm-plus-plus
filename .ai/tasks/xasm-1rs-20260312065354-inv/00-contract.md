# Contract: Investigate bin/du — 1491 diffs, +2B

**Task ID:** xasm-1rs
**Created:** 2026-03-12
**Workflow:** Bugfix (or Research if source change)
**Requires Review:** false

## Problem

`bin/du` has 1491 byte diffs and is +2B larger than stable. The +2B pattern matches
the x.fileenum shared source change seen in 12 other files (attr, chaux, etc.), but
needs confirmation. If it IS x.fileenum, all 1491 diffs should cascade from that one
2-byte insertion. If not, there may be an assembler bug.

## Files
- Source: `/tmp/A2osX-335cd122/BIN/DU.S.txt` (check exact name)
- Built: `/tmp/A2osX-335cd122-build/stage/bin/du`
- Stable: `/tmp/stable_extracted/FULL800/bin/du#062000`
- Includes: `/tmp/A2osX-335cd122/INC/`

## Investigation Method

1. Check if source includes `X.FILEENUM.S` or similar shared file
2. Map first diff position — if at offset 0x0008 (header pointer field) → x.fileenum pattern
3. Check if all subsequent diffs are cascading address adjustments from a 2-byte insertion
4. Also check for SYSCALL pattern bytes
5. Verdict: x.fileenum source change OR assembler bug

## Acceptance Criteria
```
✓ Root cause identified (x.fileenum source change or assembler bug)
✓ If assembler bug: fix implemented and ctest passes
✓ Bug tracker updated with classification
```
