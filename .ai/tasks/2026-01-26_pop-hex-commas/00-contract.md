...
Enhance HEX directive to parse comma-separated byte lists. This is the SECONDARY blocker for Prince of Persia compatibility (6 files blocked).

### Background and Context

**Analysis Finding:** The PoP analysis identified HEX comma-separated values as blocker #2:
- 114 occurrences across 6 files
- Files use: `hex 00,0d,0b,09` (comma-separated)
- Error: "stoul: no conversion"

This is a simple parser enhancement with high impact (6 files unblocked).

### Current State

HEX directive only supports concatenated format:
- ✅ Works: `hex 010203` (concatenated)
- ❌ Fails: `hex 01,02,03` (comma-separated)

### Desired State

HEX directive supports both formats:
- ✅ Works: `hex 010203` (concatenated, unchanged)
- ✅ Works: `hex 01,02,03` (comma-separated)
- ✅ Works: `hex 01, 02, 03` (with whitespace)
...
```
✓ HEX directive parses comma-separated byte lists
✓ HEX directive still parses concatenated format
✓ BOOT.S assembles successfully (has 8 comma-HEX lines)
✓ 6 previously failing PoP files now assemble (BGDATA.S, BOOT.S, GAMEBG.S, GRAFIX.S, MASTER.S, AUTO.S)
✓ All existing tests passing
✓ New tests added for HEX comma parsing
...
□ Strip whitespace around each token
□ Parse each token as hex value
□ Test: "hex 01,02,03" → emits 0x01 0x02 0x03
□ Test: "hex 010203" → emits 0x01 0x02 0x03 (unchanged)
```
...
□ Backward compatible (concatenated format still works)
□ Clear error messages for invalid hex values
...
- New concepts/patterns: No (standard string parsing)
- Integration complexity: Low (isolated change in handle_HEX())
- Risk level: Low (easy to test both formats)
