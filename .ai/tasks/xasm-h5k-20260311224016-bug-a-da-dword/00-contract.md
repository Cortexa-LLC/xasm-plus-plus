# Contract: Bug A — `.DA >expr` emits wrong 32-bit value
...
In SCMASM, `.DA >EXPR` emits the full 32-bit (DWORD) little-endian value of EXPR.
xasm++ currently emits the high byte of EXPR as a 16-bit word, producing wrong output.

### Example (from SSC.DRV.S.txt baud table)
```
P1.BAUD .DA >110    ; should emit: 6E 00 00 00 (110 = $6E)
                    ; xasm++ emits: 6E 6E (high byte repeated as 16-bit)
```

### Impact
28 diffs in ssc.drv, 28 diffs in ssc.i.drv (baud rate table + DCB.BAUD field).
...
## Acceptance Criteria
```
✓ .DA >110  emits 6E 00 00 00 (4 bytes, 32-bit LE)
✓ .DA >$1000 emits 00 10 00 00
✓ cmake --build build exits 0
✓ ctest exits 0 (all tests pass)
✓ ssc.drv baud table diffs reduced by 28
```
