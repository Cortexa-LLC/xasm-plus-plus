# Contract: Investigate sbin/cifsd — 26 diffs, same size
...
`sbin/cifsd` has 26 byte diffs vs stable at same size (+0B). This is likely SYSCALL
address changes but hasn't been verified. 26 diffs = 13 pairs if all SYSCALL.

## Files
- Source: `/tmp/A2osX-335cd122/SBIN/CIFSD.S.txt` (check exact name)
- Built: `/tmp/A2osX-335cd122-build/stage/sbin/cifsd`
- Stable: `/tmp/stable_extracted/FULL800/sbin/cifsd#062000`

## Investigation Method

1. Map all 26 diff positions (skip 16-byte header, base $2000)
2. Check SYSCALL pattern: $1000→$0140, $1003→$0143, $1010→$0150, $E200→$0153
3. If all match → SYSCALL-only, document in bug tracker and close
4. If any don't match → investigate further

## Acceptance Criteria
```
✓ All 26 diffs classified
✓ Bug tracker updated (add to SYSCALL-only list or document as new bug)
✓ If assembler bug: fix + ctest passes
```
