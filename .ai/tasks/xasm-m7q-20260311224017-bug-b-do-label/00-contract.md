# Contract: Bug B — Label on `.DO` directive line gets wrong address
...
A label placed on the same line as a `.DO` conditional directive gets assigned the wrong address.

### Example (from SSC.DRV.S.txt)
```
DIB  .DO SSCIRQ=1   ; DIB should = address of next instruction after this line
```
xasm++ assigns `DIB = $2478` (ssc.drv) but correct value is `$2480` (8 bytes higher).
The label should point to the PC value *after* any data emitted before this conditional,
not from within the conditional evaluation.

### Impact
6 diffs in ssc.drv, 6 diffs in ssc.i.drv (all instructions referencing DIB have wrong address bytes).

## Files to Modify
- `src/syntax/scmasm_directive_handlers.cpp` or `scmasm_syntax.cpp` — `.DO` label handling
- `tests/unit/test_scmasm_syntax.cpp` — add regression test

## Acceptance Criteria
```
✓ Label on .DO line resolves to correct PC (same as next instruction address)
✓ cmake --build build exits 0
✓ ctest exits 0 (all tests pass)
✓ ssc.drv DIB-related diffs reduced by 6
```
