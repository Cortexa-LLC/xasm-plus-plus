# Contract: Investigate bin/mv — 1042 diffs, same size
...
`bin/mv` has 1042 byte diffs vs stable but identical size (+0B). This is too many diffs
for pure SYSCALL address changes ($1000→$0140 = 2 bytes per call). Needs investigation.

## Files
- Source: `/tmp/A2osX-335cd122/BIN/MV.S.txt` (check exact name)
- Built: `/tmp/A2osX-335cd122-build/stage/bin/mv`
- Stable: `/tmp/stable_extracted/FULL800/bin/mv#062000`
- Includes: `/tmp/A2osX-335cd122/INC/`
- Stage area: `/tmp/a2osx-stage-oa9/` (has lowercase symlinks)

## Investigation Method

1. Map all diff positions (skip 16-byte ProDOS header, base addr $2000)
2. Check SYSCALL pattern: $1000→$0140, $1003→$0143, $1010→$0150, $E200→$0153
3. Check for other patterns (branch displacements, ZP addresses, data values)
4. If non-SYSCALL diffs found: locate in source, identify assembler bug
5. If all SYSCALL: document and close as source change

## Acceptance Criteria
```
✓ All 1042 diffs classified (SYSCALL / source change / assembler bug)
✓ If assembler bug: fix implemented, ctest passes
✓ Bug tracker updated
