...
### Completed
- ✅ Phase 1: Added DirectiveRegistry infrastructure
  - Added DirectiveHandler function signature to header
  - Added directive_registry_ member variable
  - Added InitializeDirectiveRegistry() method declaration and skeleton
  - Called InitializeDirectiveRegistry() from constructor
  - All 91 tests still passing

### In Progress
- Phase 2: Migrating individual directive handlers to registry

### Next Steps
- Register .OR directive in registry
- Register .EQ directive in registry
- Continue with remaining directives

### Phase 2 Completed
- ✅ Registered all directives in DirectiveRegistry:
  - Simple: .OR, .EQ, .SE
  - String: .AS, .AT, .AZ
  - Data: .DA, .DFB (alias), .HS, .BS
  - Macro: .MA, .ENDM
  - Note: Control flow directives (.DO, .ELSE, .FIN, .LU, .ENDU) NOT registered
    because they require special handling in ParseLine (line skipping, nesting)
- All 91 tests still passing

### In Progress
- Phase 3: Refactor ParseLine to use directive registry

### Phase 3 Completed
- ✅ Refactored ParseLine to dispatch directives via registry
- ✅ Replaced giant if-else chain with registry lookup
- ✅ Special handling preserved for:
  - Label validation (.EQ, .SE require labels)
  - Control flow directives (.DO, .LU, .ELSE, .FIN, .ENDU)
- ✅ Added .EM as alias for .ENDM
- All 91 SCMASM tests passing
- Full build successful with zero warnings

### Pattern Applied
- Directive handling now uses registry pattern for extensibility
- New directives can be added by registering in InitializeDirectiveRegistry()
- Control flow directives remain special-cased (by design - require line skipping)
...
- ✅ Docstrings complete (C++ comments)
- ✅ Tests written and passing (TDD workflow followed)
