# Contract: Run 15 — Full A2osX Rebuild and Binary Comparison
...
Full clean A2osX rebuild + binary comparison against stable reference (commit ce94435).

## Procedure

1. `cp build/bin/xasm++ /tmp/xasm_new`
2. Clean stage: `find /tmp/A2osX-335cd122-build/stage -not -path "*/inc/*" -not -path "*/usr/src/*" -type f -delete`
3. Rebuild: `cmake --build /tmp/A2osX-335cd122-build -- -k`
4. Compare all files in stage vs `/tmp/stable_extracted/FULL800/` (skip 16-byte ProDOS header)
5. Report: identical / different / not_built counts with per-file details
6. Update `.ai/docs/a2osx-bug-tracker.md` with Run 15 row (include commit ce94435)
7. Document any newly identical files and any unexplained new diffs

## Context
- Run 14 baseline: 74 identical, 42 different, 20 not_built
- Fixes in ce94435: Bugs 16-20 (double-dot label, infix multiply, .DA >DWORD, .DO label off-by-1, .DA #char)
- ssc.drv and ssc.i.drv confirmed byte-identical after fixes

## Acceptance Criteria
```
✓ Full rebuild completes
✓ Comparison report produced
✓ Bug tracker updated with Run 15 row
✓ Newly identical files listed
```
