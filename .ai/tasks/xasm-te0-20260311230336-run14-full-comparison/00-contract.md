# Contract: Run 14 — Full A2osX Rebuild and Binary Comparison
...
Run 14: Full A2osX rebuild + binary comparison against stable reference to measure improvement from xasm-zp1 fixes (Bugs A/B/C + double-dot label + infix multiply).

### Background

Run 13 (xasm-g8i): 71 identical, 53 different, 89 not built.
Since then, 5 bugs fixed:
- Double-dot label mis-parse (`X.BasePath..1`)
- Infix `*` multiplication in ParseExpression
- Bug A: `.DA >expr` 32-bit DWORD encoding
- Bug B: Label on `.DO` directive line wrong address
- Bug C: `.DA #'char'` emits 0x00

Expected improvement: ssc.drv and ssc.i.drv should now match stable (36 fewer bug-diffs each, 3 remain as source changes). Potentially 2 more identical files.

### Procedure

Standard Run 14 comparison procedure:
1. `cp build/bin/xasm++ /tmp/xasm_new`
2. Clean the A2osX stage: `find /tmp/A2osX-335cd122-build/stage -not -path "*/inc/*" -not -path "*/usr/src/*" -type f -delete`
3. Rebuild A2osX: `cmake --build /tmp/A2osX-335cd122-build -- -k`
4. Run comparison script: `cd /tmp && python3 run_compare_v2.py`
5. Update `.ai/docs/a2osx-bug-tracker.md` with Run 14 results

### Stable Reference
- `/tmp/stable_extracted/` (STABLE.800.po extracted files)
- Comparison script: `/tmp/run_compare_v2.py`

## Acceptance Criteria
```
✓ Full A2osX rebuild completes (errors expected for known not-built files)
✓ Comparison script produces identical/different/not_built counts
✓ Bug tracker updated with Run 14 row
✓ Any newly identical files documented
✓ Any unexpected new diffs investigated
```
