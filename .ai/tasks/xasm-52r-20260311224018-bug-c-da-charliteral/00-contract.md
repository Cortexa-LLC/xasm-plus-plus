# Contract: Bug C — `.DA #'char'` emits 0x00 instead of ASCII value
...
`.DA #'N'` should emit the ASCII value of 'N' (0x4E) as a 1-byte immediate, but xasm++ emits 0x00.

### Example (from SSC.DRV.S.txt)
```
DCB.PARITY  .DA #'N'   ; should emit: 4E ('N' = ASCII 78 = $4E)
DCB.FLOW    .DA #'N'   ; should emit: 4E
            ; xasm++ emits: 00 00
```

### Impact
2 diffs in ssc.drv, 2 diffs in ssc.i.drv (DCB.PARITY and DCB.FLOW fields).

## Investigation Hints
- The `#` prefix in `.DA #value` means emit 1 byte (immediate form)
- The `'char'` form is a character literal — ASCII value of the char
- The `#` + `'char'` combination may not be handled in `HandleDa()`
- Check `ParseExpression` or `HandleDa` for char literal + immediate interaction

## Files to Modify
- `src/syntax/scmasm_directive_handlers.cpp` — `HandleDa()` with `#'char'` form
- `tests/unit/test_scmasm_syntax.cpp` — add regression tests

## Acceptance Criteria
```
✓ .DA #'N'  emits 4E (1 byte)
✓ .DA #'A'  emits 41 (1 byte)
✓ .DA #$FF  still emits FF (existing behavior preserved)
✓ cmake --build build exits 0
✓ ctest exits 0 (all tests pass)
✓ ssc.drv DCB.PARITY/FLOW diffs resolved
```
