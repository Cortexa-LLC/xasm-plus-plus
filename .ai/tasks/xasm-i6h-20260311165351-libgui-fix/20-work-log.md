...
### Objectives
- Investigate libgui -132B delta vs STABLE reference
- Find root cause and implement fix

### Work Completed

#### Investigation
1. Generated assembly listing for LIBGUI.S.txt
2. Binary-compared built vs stable: first diff at offset 0x0008 (pointer table)
3. Traced pointer table deltas: found +132 byte swing at BM.INFO function

#### Root Cause Found: .HX Directive Bug

**Bug:** `HandleHx()` in `src/syntax/scmasm_directive_handlers.cpp` treated each hex digit as a separate byte, producing 8 bytes per `.HX 00000000` instead of 4.

**SCMASM .HX semantics:**
- Takes pairs of hex digits
- For pair "XY": stores byte `(Y << 4) | X` (low nibble first, high nibble second)
- This is nibble-swapped vs standard hex notation
- `.HX 00022000` → bytes `00 20 02 00` (NOT `00 02 20 00`)
- This reflects Apple II 4bpp color pixel storage convention

**Impact:** BM.Info in LIBGUI.G.BM.txt uses 14 rows of `.HX` directives.
- Wrong: each `.HX 00000000` produced 8 bytes (one per nibble digit)
- Correct: each `.HX 00000000` produces 4 bytes (one per nibble pair)
- 14 rows × 4 bytes difference = 56 bytes over per section × 2 sections (data + AND) = 112 extra bytes
- Plus header AND_offset was 0x78 (120 bytes) instead of 0x40 (64 bytes)

#### Fix Implemented
- Fixed `HandleHx()` in `src/syntax/scmasm_directive_handlers.cpp`:
  1. Collect all nibbles first
  2. Pair consecutive nibbles: first nibble → LOW nibble, second → HIGH nibble
  3. `byte = (high_nibble << 4) | low_nibble`
- Added 6 unit tests in `tests/unit/test_scmasm_syntax.cpp` covering:
  - All-zero pattern
  - Nibble-swap behavior
  - BM.Info row encoding (00022000 → 00 20 02 00)
  - FFF00FFF pattern (FF 0F F0 FF)
  - Size verification (AABB → 2 bytes, not 4)
  - Lowercase hex digits

#### Results
- Before fix: built=10912B, stable=10780B, diff=+132B
- After fix:  built=10800B, stable=10780B, diff=+20B
- All 1834 tests pass (1828 original + 6 new .HX tests)

#### Remaining 20-byte difference (SOURCE-LEVEL, not assembler bug)
1. **+22 bytes**: `BM.Checked` bitmap was added in commit 9a7d20a7 (along with .HX support)
   but the stable reference binary was assembled from a pre-9a7d20a7 version without it.
   This is an expected source-level difference between 335cd122 and the stable-era source.

2. **-2 bytes**: `KBD` and `KBDSTROBE` symbols are used in LIBGUI.S.MSG.txt but are not
   defined in 335cd122's IO.I (they were renamed to IO.KBD/IO.KBDSTROBE between commits).
   The original SCMASM assembled these as absolute $C000/$C010 (3-byte instructions).
   xasm++ resolves undefined symbols as $0000 → zero-page (2-byte instructions).
   This is the "IO-prefix rename" source bug noted in the task background.

### Files Modified
- `src/syntax/scmasm_directive_handlers.cpp` - Fixed HandleHx()
- `tests/unit/test_scmasm_syntax.cpp` - Added 6 .HX tests

### Status: COMPLETE (assembler bug fixed, residual is source-level issue)
