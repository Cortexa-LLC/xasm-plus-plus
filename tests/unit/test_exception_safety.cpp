...
TEST_F(ExceptionSafetyTest, Assembler_EncodingError_PreservesContext) {
    ...
    // If it succeeds, that's okay - we're testing exception handling, not BRK semantics
...
TEST_F(ExceptionSafetyTest, CPU_InvalidOpcode_SpecificException) {
    ...
    // The important thing is it doesn't crash
    try {
        std::vector<uint8_t> bytes = cpu->EncodeLDA(0x1234, AddressingMode::Indirect);
        ...
        FAIL() << "Should throw specific exception type or succeed, not throw unknown exception";
    }
}
...
TEST_F(ExceptionSafetyTest, CPU_OutOfRange_SpecificException) {
    ...
    try {
        std::vector<uint8_t> bytes = cpu->EncodeBranchWithRelaxation(
            0xF0,  // BEQ opcode
            from_addr,
            to_addr
        );
