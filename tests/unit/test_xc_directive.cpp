...
TEST(XcDirectiveTest, Cpu65C02InstructionWithoutXcReturnsEmptyVector) {
    ...
    EXPECT_TRUE(bytes_phy.empty()) << "PHY should return empty vector in 6502 mode";

    auto bytes_plx = cpu.EncodePLX();
    EXPECT_TRUE(bytes_plx.empty()) << "PLX should return empty vector in 6502 mode";

    auto bytes_tsb = cpu.EncodeTSB(0x80, AddressingMode::ZeroPage);
    EXPECT_TRUE(bytes_tsb.empty()) << "TSB should return empty vector in 6502 mode";
}
