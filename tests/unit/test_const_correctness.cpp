...
// Addresses Minor Issue N1: Add const qualifiers to methods that don't modify
// object state
...
TEST(ConstCorrectnessTest, SymbolTableIsDefinedIsConst) {
  ConcreteSymbolTable symbols;
  symbols.Define("test", SymbolType::Label,
                 std::make_shared<LiteralExpr>(0x3000));
...
TEST(ConstCorrectnessTest, SymbolTableGetAllSymbolsIsConst) {
  ConcreteSymbolTable symbols;
  symbols.Define("sym1", SymbolType::Label,
                 std::make_shared<LiteralExpr>(0x100));
  symbols.Define("sym2", SymbolType::Equate,
                 std::make_shared<LiteralExpr>(0x200));
...
TEST(ConstCorrectnessTest, ConstMethodsWorkInRealWorldScenario) {
  ...
  mutable_symbols.Define("start", SymbolType::Label,
