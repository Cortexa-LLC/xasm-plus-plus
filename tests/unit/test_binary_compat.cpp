...
// Test 1: File with only labels and equates (no code/data) should produce 0 bytes
TEST(BinaryCompatTest, EquateOnlyFileProducesZeroBytes) {
  ...
  EXPECT_EQ(file_size, 0) << "Equate-only file should produce 0-byte output (vasm compatibility)";
...
TEST(BinaryCompatTest, FileWithDataProducesNormalOutput) {
  ...
  section.atoms.push_back(std::make_shared<DataAtom>(std::vector<uint8_t>{0x03, 0x04}));
...
TEST(BinaryCompatTest, HighOrgNoZeroPadding) {
  ...
  section.atoms.push_back(std::make_shared<DataAtom>(std::vector<uint8_t>{0xE9, 0x30}));
  ...
  ASSERT_EQ(result.size(), 2) << "Should not pad from $0000 to ORG (vasm compatibility)";
...
TEST(BinaryCompatTest, OrgLabelsDataNoPadding) {
  ...
  section.atoms.push_back(std::make_shared<OrgAtom>(0x8000));
  section.atoms.push_back(std::make_shared<LabelAtom>("start", 0x8000));
  section.atoms.push_back(std::make_shared<DataAtom>(std::vector<uint8_t>{0x01, 0x02, 0x03}));
...
label2 ds 20
 dend
  ...
  EXPECT_EQ(file_size, 0) << "DS inside DUM block should not emit bytes (vasm compatibility)";
...
// Test 9: ASC directive should set high bit on all characters (Apple II standard)
TEST(BinaryCompatTest, AscSetsHighBit) {
  // Apple II text uses "high-bit ASCII" where all text characters have bit 7 set
  // 'A' = 0x41 → 0xC1, 'B' = 0x42 → 0xC2, etc.
 ...
 asc "ABC"
  ...
  ASSERT_EQ(result.size(), 3);
  EXPECT_EQ(result[0], 0xC1) << "ASCII 'A' (0x41) should become 0xC1 (high-bit set)";
  EXPECT_EQ(result[1], 0xC2) << "ASCII 'B' (0x42) should become 0xC2 (high-bit set)";
  EXPECT_EQ(result[2], 0xC3) << "ASCII 'C' (0x43) should become 0xC3 (high-bit set)";
...
TEST(BinaryCompatTest, AscPrinceOfPersiaHighBit) {
 ...
 asc "Prince of Persia"
  ...
  EXPECT_EQ(result[0], 0xD0) << "Expected 0xD0 (not 0x50) for 'P' with high bit";
  EXPECT_EQ(result[1], 0xF2) << "Expected 0xF2 (not 0x72) for 'r' with high bit";
  EXPECT_EQ(result[2], 0xE9) << "Expected 0xE9 (not 0x69) for 'i' with high bit";
  EXPECT_EQ(result[3], 0xEE) << "Expected 0xEE (not 0x6E) for 'n' with high bit";
...
seq3     db $03
  ...
  ASSERT_EQ(result.size(), 9) << "Should have 6 bytes (3 words) + 3 bytes (data)";
