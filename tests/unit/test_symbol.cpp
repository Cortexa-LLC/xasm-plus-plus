...
TEST(SymbolTest, SymbolTableMultipleSymbols) {
    ...
    table.Define("label1", SymbolType::Label, std::make_shared<LiteralExpr>(0x1000));
    table.Define("label2", SymbolType::Label, std::make_shared<LiteralExpr>(0x2000));
...
TEST(SymbolTest, SymbolTableIteration) {
    ...
    for (const auto& [name, sym] : symbols) {
        if (name == "label1" && sym.type == SymbolType::Label) found_label1 = true;
        if (name == "label2" && sym.type == SymbolType::Label) found_label2 = true;
        if (name == "CONST" && sym.type == SymbolType::Equate) found_const = true;
    }
