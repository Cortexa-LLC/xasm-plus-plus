...
protected:
    ...
    void CompareWithReference(const BinaryCompatTestCase& test_case) {
        ...
        std::string xasm_cmd = xasm_path + " "
            ...
            "--cpu=" + test_case.cpu + " "
            "-o " + temp_output_path + " "
            + test_source_dir + "/" + test_case.source_file;

        // Run xasm++
        int result = std::system(xasm_cmd.c_str());
        ASSERT_EQ(result, 0) << "xasm++ failed to assemble " << test_case.source_file;
        ...
        std::string source_filename = test_case.source_file.substr(test_case.source_file.find_last_of('/') + 1);
        source_filename = source_filename.substr(0, source_filename.length() - 4) + ".bin"; // Replace .asm with .bin
        ...
        ASSERT_FALSE(vasm_binary.empty()) << "Golden reference file not found: " << golden_path;
