...
ErrorFormatter::ErrorFormatter(ColorMode mode)
    : color_mode_(mode) {}
...
std::string ErrorFormatter::FormatError(
    ...
    if (!error.location.filename.empty()) {
        ...
        // Source context
        std::string source_line = ReadSourceLine(error.location.filename, 
                                                  error.location.line);
        if (!source_line.empty()) {
            ...
            std::string marker = GenerateColumnMarker(error.location.column,
                                                      marker_length,
                                                      "");
            oss << marker;
...
std::string ErrorFormatter::GenerateColumnMarker(size_t column,
                                                 size_t length,
...
std::vector<std::string> ErrorFormatter::FindSimilarSymbols(
...
std::string ErrorFormatter::ExtractSymbolName(const std::string& message) const {
...
std::string ErrorFormatter::FormatSuggestions(
    ...
    if (suggestions.size() == 1) {
        // Single suggestion
        if (ShouldUseColors()) {
            oss << "   \033[1;32mhelp:\033[0m did you mean '" 
                << suggestions[0] << "'?\n";
        } else {
