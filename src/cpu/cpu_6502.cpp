 ...
 */
std::vector<uint8_t> Cpu6502::EncodeWithTable(const OpcodeTable& table, uint32_t operand,
                                              AddressingMode mode, ImmWidth imm_width) const {
  ...
  if (!opcode.has_value() && mode == AddressingMode::ZeroPage && table.absolute.has_value()) {
    opcode = table.absolute;
    mode = AddressingMode::Absolute;
  }

  // ZeroPageX → AbsoluteX fallback: same reasoning for indexed X mode.
  if (!opcode.has_value() && mode == AddressingMode::ZeroPageX && table.absolute_x.has_value()) {
    opcode = table.absolute_x;
  ...
  if (!opcode.has_value() && mode == AddressingMode::ZeroPageY && table.absolute_y.has_value()) {
    opcode = table.absolute_y;
...
std::vector<uint8_t> Cpu6502::EncodeJMP(uint16_t operand, AddressingMode mode) const {
  static const OpcodeTable kJmpTable = {.immediate = std::nullopt,
...
size_t Cpu6502::GetInstructionSize(const std::string& mnemonic,
  ...
  if (kOp[0] == '(') {
    ...
    if (close != std::string::npos) {
      ...
      if (close + 1 < kOp.length()) {
        const std::string kAfterUpper = to_upper(util::Trim(kOp.substr(close + 1)));
        if (kAfterUpper == ",Y") {
          return 2;
        }
      }
  ...
  const size_t kCommaPos = (kCommaX != std::string::npos) ? kCommaX : kCommaY;
...
std::vector<uint8_t> Cpu6502::EncodeBRA(uint16_t operand, AddressingMode mode) const {
  ...
  static const OpcodeTable kBraTable = {.immediate = std::nullopt,
                                        .zero_page = std::nullopt,
                                        .zero_page_x = std::nullopt,
                                        .zero_page_y = std::nullopt,
                                        .absolute = std::nullopt,
...
void Cpu6502::SetRelaxBranches(bool relax) {
  relax_branches_ = relax;
}
...
using EncFnAlias = std::function<std::vector<uint8_t>(const Cpu6502*, uint32_t, AddressingMode)>;
 ...
 */
std::vector<uint8_t> Cpu6502::EncodeInstruction(const std::string& mnemonic, uint32_t operand,
                                                const std::string& operand_str) const {
  // Strip trailing '!' from mnemonic before processing
 ...
 */
std::vector<uint8_t> Cpu6502::EncodeInstructionSpecial(const std::string& mnemonic,
