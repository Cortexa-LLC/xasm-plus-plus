...
// actual code size).
static void WriteRw18Header(std::ofstream &out,
                             const std::array<uint16_t, 4> &rw18_args,
                             uint32_t code_size) {
...
static void WriteAtom(std::ofstream &out, const std::shared_ptr<Atom> &atom,
                      size_t &position) {
  switch (atom->type) {
  case AtomType::Data: {
...
void BinaryOutput::WriteOutputWithRw18(
    const std::string &filename, const std::vector<Section *> &sections,
    const SymbolTable &symbols,
