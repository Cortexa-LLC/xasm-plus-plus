...
static std::string FormatAddress(uint32_t addr, int width = 4) {
  ...
  oss << std::uppercase << std::hex << std::setw(width) << std::setfill('0') << addr;
  return oss.str();
}
 ...
 * @return Formatted string
 */
static std::string FormatBytes(const std::vector<uint8_t>& bytes, size_t max_bytes = 8) {
  std::ostringstream oss;
  ...
  for (const auto* section : sections) {
    ...
    for (const auto& atom : section->atoms) {
      ...
      if (auto* label = dynamic_cast<const LabelAtom*>(atom.get())) {
        ...
        file << FormatAddress(current_address) << "     "
             << std::left << std::setw(17) << bytes_str << "  "
             << inst->mnemonic;
