...
void TrsDosWriter::Write(const std::vector<Section>& sections, std::ostream& output) {
...
std::string TrsDosWriter::GetExtension() const {
  return "cmd";
}

std::string TrsDosWriter::GetFormatName() const {
...
void TrsDosWriter::WriteSegment(std::ostream& output, uint8_t type, uint64_t address,
...
std::vector<std::pair<uint64_t, uint8_t>> TrsDosWriter::ExtractBytes(const Section& section) {
