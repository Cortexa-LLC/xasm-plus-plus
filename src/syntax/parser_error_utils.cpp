...
void ThrowFormattedError(const DirectiveContext& ctx, const std::string& message) {
  throw std::runtime_error(FormatError(ctx, message));
}

void ThrowRequiresLabel(const DirectiveContext& ctx, const std::string& directive_name) {
  std::string message = directive_name + " requires a label";
  throw std::runtime_error(FormatError(ctx, message));
}

void ThrowRequiresOperand(const DirectiveContext& ctx, const std::string& directive_name) {
...
void ThrowNotImplemented(const DirectiveContext& ctx, const std::string& directive_name,
...
void ThrowInvalidValue(const DirectiveContext& ctx, const std::string& value_type,
