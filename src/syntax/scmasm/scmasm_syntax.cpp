...
ScmasmSyntaxParser::ScmasmSyntaxParser()
    : current_address_(0)
    , current_file_("<source>")
    , current_line_(0)
    , in_macro_definition_(false)
    , macro_invocation_depth_(0)
{
}
...
void ScmasmSyntaxParser::Parse(const std::string& source, Section& section, ConcreteSymbolTable& symbols) {
...
void ScmasmSyntaxParser::ParseLine(const std::string& line, Section& section, ConcreteSymbolTable& symbols,
    ...
    // For directives starting with ., stop at whitespace (directives are like .OR, .EQ)
    // But NOT for local labels like .1 which appear in operands
    while (pos < line.length() && !std::isspace(line[pos])) {
        pos++;
    }
    ...
    if (!opcode.empty() && opcode[0] == '.') {
        ...
        else if (opcode_upper == ".ELSE" || opcode_upper == ".FIN" || opcode_upper == ".ENDU") {
        ...
        if (it != macros_.end()) {
            ...
            // Phase 3: We don't parse instructions yet, just store them as InstructionAtom
            auto instr_atom = std::make_shared<InstructionAtom>(opcode_upper, operand);
            section.atoms.push_back(instr_atom);
            
            // Assume each instruction is 1-3 bytes (will be properly sized by CPU plugin later)
            // For now, advance by a conservative 3 bytes
            current_address_ += 3;
        }
    }
}

std::string ScmasmSyntaxParser::ParseLabel(const std::string& line, size_t& pos, Section& /*section*/, ConcreteSymbolTable& /*symbols*/) {
    // Skip leading whitespace
    while (pos < line.length() && std::isspace(line[pos])) {
        pos++;
    }
...
void ScmasmSyntaxParser::HandleOr(const std::string& operand, Section& section, ConcreteSymbolTable& symbols) {
    if (operand.empty()) {
...
void ScmasmSyntaxParser::HandleEq(const std::string& label, const std::string& operand, ConcreteSymbolTable& symbols) {
    if (operand.empty()) {
...
void ScmasmSyntaxParser::HandleSe(const std::string& label, const std::string& operand, ConcreteSymbolTable& symbols) {
    if (operand.empty()) {
...
bool ScmasmSyntaxParser::IsLocalLabel(const std::string& label) {
    ...
    if (label.length() == 2 && label[0] == '.' && label[1] >= '0' && label[1] <= '9') {
        return true;
    }
    return false;
}

uint32_t ScmasmSyntaxParser::EvaluateExpression(const std::string& str, ConcreteSymbolTable& symbols) {
    std::string trimmed = Trim(str);
...
std::shared_ptr<Expression> ScmasmSyntaxParser::ParseExpression(const std::string& str, ConcreteSymbolTable& /*symbols*/) {
...
void ScmasmSyntaxParser::HandleBs(const std::string& operand, Section& section,
    ...
    for (char c : trimmed) {
        if (!std::isspace(c)) {
            if (c != '0' && c != '1') {
                throw std::runtime_error("Invalid binary digit in .BS: " + std::string(1, c));
            }
            bin_digits += c;
        }
    }
...
void ScmasmSyntaxParser::HandleMa(const std::string& label, const std::string& operand) {
...
void ScmasmSyntaxParser::HandleDo(const std::string& operand, 
                                   Section& section,
                                   ConcreteSymbolTable& symbols,
    ...
    for (size_t i = line_idx + 1; i < source.size(); ++i) {
        ...
        std::string directive = line.substr(directive_start, token_pos - directive_start);
...
void ScmasmSyntaxParser::HandleLu(const std::string& operand,
                                   Section& section,
                                   ConcreteSymbolTable& symbols,
                                   const std::vector<std::string>& source,
                                   size_t& line_idx) {
